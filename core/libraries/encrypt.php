...
class encrypt
{
    private static $auth_key='fe5a-36f5-225e-af2e';
    ...
    {
        self::$auth_key = self::$auth_key?:md5('authkey');
        $ckey_length = 4; // 动态密匙长度，相同的明文会生成不同密文就是依靠动态密匙
        $key = md5($key ? $key : self::$auth_key); // 密匙
    ...
    public static function encrypt($string, $operation='E', $key='') {
        $key = md5($key);
